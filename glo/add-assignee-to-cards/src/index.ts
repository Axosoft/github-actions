...
async function run() {
  ...
  try {
    ...
    for (let i=0; i < cards.length; i++) {
      ...
      const board = await GloSDK(authToken).boards.get(boardId, { fields: ['members'] });
      if (!board) {
      ...
      const card = await GloSDK(authToken).boards.cards.get(boardId, cardId, { fields: ['assignees'] });
      if (!card) {
