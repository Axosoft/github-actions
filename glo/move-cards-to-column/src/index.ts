...
async function run() {
  ...
  try {
    ...
    for (let i=0; i < cards.length; i++) {
      ...
      const board = await GloSDK(authToken).boards.get(boardId, { fields: ['columns'] });
      if (!board) {
      ...
      const card = await GloSDK(authToken).boards.cards.get(boardId, cardId, { fields: ['column_id'] });
      if (!card) {
